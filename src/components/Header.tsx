...
function LangToggle({ className = "" }: { className?: string }) {
  ...
  return (
    <div
      className={`flex items-center rounded-full border border-ink/15 p-0.5 font-sans text-[11px] font-medium ${className}`}
    >
      {(["fr", "en"] as const).map((l) => (
        <button
          ...
          className={`rounded-full px-2.5 py-1 uppercase tracking-wider transition-colors ${
            lang === l ? "bg-ink text-ivory" : "text-ink hover:text-amber"
          }`}
        >
          {l}
        </button>
    ...
    <>
      ...
      >
        ...
        <div className="container-luxe flex h-[80px] items-center justify-between">
          <Link href="/" aria-label="Parfumarium" className="group shrink-0">
            <Logo tone="gold" variant="icon" size={40} className="transition-opacity group-hover:opacity-80" />
          </Link>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <button
              ...
              className="flex h-10 w-10 items-center justify-center rounded-full border border-transparent text-ink transition-all duration-300 hover:border-gold/40 hover:text-amber sm:h-11 sm:w-11"
            >
            ...
            <Link
              ...
              className="hidden h-11 w-11 items-center justify-center rounded-full border border-transparent text-ink transition-all duration-300 hover:border-gold/40 hover:text-amber sm:flex"
            >
              <svg width="21" height="21" viewBox="0 0 24 24" fill="none">
            ...
            <Link
              ...
              className="relative flex h-10 w-10 items-center justify-center rounded-full border border-transparent text-ink transition-all duration-300 hover:border-gold/40 hover:text-amber sm:h-11 sm:w-11"
            >
              ...
              {count > 0 && (
                <span className="absolute -right-0.5 -top-0.5 flex h-5 min-w-5 items-center justify-center rounded-full bg-gold px-1 font-sans text-[11px] font-medium text-ink">
                  {count}
                </span>
              )}
            </Link>
            ...
            >
              <span className="relative block h-3.5 w-6">
                <span className={`absolute left-0 block h-px w-full bg-ink transition-all duration-300 ${menuOpen ? "top-1.5 rotate-45" : "top-0"}`} />
                <span className={`absolute left-0 top-1.5 block h-px w-full bg-ink transition-all duration-300 ${menuOpen ? "opacity-0" : "opacity-100"}`} />
                <span className={`absolute left-0 block h-px w-full bg-ink transition-all duration-300 ${menuOpen ? "top-1.5 -rotate-45" : "top-3"}`} />
              </span>
            </button>
          </div>
        </div>
      ...
      <Drawer open={menuOpen} onClose={() => setMenuOpen(false)} side="right" title={t.header.menu}>
        ...
        <div className="mt-4 space-y-5 border-t border-champagne px-6 py-6">
          <button
            type="button"
            onClick={() => {
              setMenuOpen(false);
